...
from puppies_db_setup import Base, Puppy, Shelter
...
puppies = session.query(Puppy).filter(Puppy.dateOfBirth > sixMonthsAgo).order_by((asc('puppy.dateOfBirth')))
print ('\n\nBEGIN ACCEPTABLE LIST - PUPPIES LESS THAN SIX MONTHS OLD\n')
for p in puppies:
	print ('Hi. %s here, all young and peppy. I was born %s.'%(p.name, p.dateOfBirth))
...
for p in puppies:
	print ('Hi again. I hope I am not a fat puppy. My name is %s. I weigh %s.'%(p.name,p.weight))
